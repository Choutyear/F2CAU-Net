...
class FuzzyUNet(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.fuzzy_conv = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1)
        self.fuzzy_attention = nn.Sequential(
            nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1),
            nn.Sigmoid()
        )
    ...
    def forward(self, input):
        ...
        #
        fuzzy_out = self.fuzzy_conv(enc_out)

        #
        attention_out = self.fuzzy_attention(fuzzy_out)
        weighted_out = enc_out * attention_out

        #
        cat_out = torch.cat((weighted_out, fuzzy_out), dim=1)
        dec_out = self.decoder(cat_out)
